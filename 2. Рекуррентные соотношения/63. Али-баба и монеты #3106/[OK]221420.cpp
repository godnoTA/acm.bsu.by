...
int main() {
	...
	for (int j = 1; j < n && flag; j++) {
		for (int i = 0; i < n - j && flag; i++) {
			...
			for (int k = 0; k < i && flag && flag1 && min_time_left[i][j] < MAX_NUMB; k++) {
				if (times[k] - min_time_left[i][j] < positions[i] - positions[k])
					flag1 = false;
			}
			for (int k = i + j + 1; k < n && flag && flag2 && min_time_right[i][j] < MAX_NUMB; k++) {
				if (times[k] - min_time_right[i][j] < positions[k] - positions[i + j])
					flag2 = false;
			}
			if (!flag1 && !flag2)
				flag = false;
