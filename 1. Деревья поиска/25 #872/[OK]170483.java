...
public class Work {
    ...
    static class Tree {
        Node root;
        int amount_of_nodes;
        private ArrayList<Pair<Integer,Integer>> arr;
        ...
        private void findNodeWithMaxDifference(Node node) {
            if(node.getDifference() >= max_difference){
                ...
                arr.add(new Pair<>(node.getDifference(),node.getData()));
            }
        }
        ...
        public int findMedium(ArrayList<Pair<Integer,Integer>> a){
    ...
    public static void main(String[] args) throws IOException {
            ...
            tree.arr = new ArrayList<>();
            tree.traverseTreeAndFindNodeWithMaxDifference(tree.root);
            Iterator<Pair<Integer,Integer>> iter = tree.arr.iterator();
            while (iter.hasNext()) {
                Pair<Integer, Integer> s = iter.next();
                if (max_difference > (int) s.getKey()) {
                    iter.remove();
                }
            }

            if(tree.arr.size() % 2 != 0){
                tree.delete(tree.findMedium(tree.arr));
            }
            if(!tree.isEmpty()){
