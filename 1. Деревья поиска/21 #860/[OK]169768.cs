...
namespace Alg_Lab1
{
	...
	public class BinaryTree
	{
		...
		public BinaryTree GetAverageNodeByCriteria()
		{
			List<long?> nodesByCriteria = new List<long?>();
			AddToListOfNodesByCriteria(nodesByCriteria);
			nodesByCriteria.Sort();
			if (nodesByCriteria.Count % 2 != 0)
			{
				long? dataToRemove = nodesByCriteria[nodesByCriteria.Count / 2];
				return Find(dataToRemove.Value);
			}
			return null;
		}

		private void AddToListOfNodesByCriteria(List<long?> nodes)
		{
			long leftHeight = Left != null ? Left.Height() : -1;
			long rightHeight = Right != null ? Right.Height() : -1;
			long leftCount = Left != null ? Left.CountElements() : 0;
			long rightCount = Right != null ? Right.CountElements() : 0;
			if (leftHeight == rightHeight && leftCount != rightCount)
			{
				nodes.Add(Data);
			}
			if (Left != null)
			{
				Left.AddToListOfNodesByCriteria(nodes);
			}
			if (Right != null)
			{
				Right.AddToListOfNodesByCriteria(nodes);
			}
		}
	...
	class Program
	{
		static void Main(string[] args)
		{
			...
			{
				BinaryTree dataToRemove = binaryTree.GetAverageNodeByCriteria();
				binaryTree.Remove(dataToRemove);
			}
			using (StreamWriter output = new StreamWriter("out.txt"))
			{
				binaryTree.Print(output);
			}
		}
	}
}
